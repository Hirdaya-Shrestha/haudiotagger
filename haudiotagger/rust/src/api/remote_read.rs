...
impl ReqwestHttpClient {
    ...
    fn head(&self, url: &str) -> Result<RemoteFileInfo, HaudiotaggerError> {
    ...
    fn get_all(&self, url: &str) -> Result<HttpResponse, HaudiotaggerError> {
        let response = self.retry(|| {
            self.client
                .get(url)
                .timeout(std::time::Duration::from_millis(self.request_timeout_ms))
...
fn detect_from_url(url: &str, http: &ReqwestHttpClient) -> DetectedFormat {
    // Try Content-Type header first (cheap)
    ...
    // Fall back to magic bytes (first 16 bytes)
...
impl RemoteReader {
    fn new(url: String, info: RemoteFileInfo, http: Arc<ReqwestHttpClient>) -> Self {
    ...
    fn read_at(&self, offset: u64, length: u64) -> Result<Vec<u8>, HaudiotaggerError> {
        if length == 0 {
            return Ok(Vec::new());
        }
        ...
        if !self.accept_ranges {
            // No range support — download everything once
        ...
        // Fetch block(s) covering [offset, offset+length)
...
pub fn read_from_url(url: String, strategy: UrlReadStrategy) -> Result<Tag, HaudiotaggerError> {
    // Validate URL
    ...
    match effective {
        UrlReadStrategy::Full | UrlReadStrategy::Auto => {
...
mod tests {
    ...
    fn cache_eviction() {
        ...
        // Insert enough to exceed MAX_CACHE_BYTES (16MB)
        for i in 0..2000 {
    ...
    // ── URL validation (via read_from_url) ───────────────────────────────

    #[test]
    fn read_from_url_invalid_url() {
        let result = read_from_url("not-a-url".to_string(), UrlReadStrategy::Full);
    ...
    // ── Integration: read_from_url against live server ───────────────────

    // #[test]
    // #[ignore] // requires network; run with --include-ignored
    // fn read_from_url_integration() {
    //     let url = "https://httpbin.org/get";
    //     let client = ReqwestHttpClient::new();
    //     let detected = detect_from_url(url, &client);
    //     assert_eq!(detected, DetectedFormat::Unknown);
    // }

    #[test]
    fn read_from_url_rejects_non_http() {
